...
exports.getCategorizedProducts = exports.characteristicsIcons = exports.getSingleProduct = exports.deleteProduct = exports.queryProducts = exports.deleteImage = exports.getAllProducts = exports.updateProduct = exports.createProduct = void 0;
const client_1 = require("@prisma/client");
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const data_1 = require("../../assets/data");
const base_utl_1 = require("../../utils/base_utl");
const data_2 = require("../../assets/data");
...
const buildFilterQuery = (query) => {
    const { name, brand, category, subCategory, typeOfShoes, offer, 
    // size,
    colorName, colorCode, company, gender, availability, minPrice, maxPrice, } = query;
    const where = {};
...
//     })),
//   }));
const formatProductsWithImageUrls = (products) => products.map((product) => ({
    ...product,
    ...
    colors: product.colors.map((color) => ({
        ...
        images: color.images.map((image) => ({
            ...image,
            url: (0, base_utl_1.getImageUrl)(`/uploads/${image.url}`),
        })),
    })),
}));
const mapCharacteristicsToDetails = (ids) => {
    return data_2.data
        .filter((item) => ids.includes(item.id))
        .map((item) => ({
        id: item.id,
        text: item.text,
        image: (0, base_utl_1.getImageUrl)(`/assets/KeinTitel/${item.image}`),
    }));
};
// ----------------------------------------------------------
const createProduct = async (req, res) => {
    try {
        const { name, brand, Category, Sub_Category, typeOfShoes, productDesc, price, availability, offer, size, feetFirstFit, footLength, technicalData, Company, gender, colors, question, characteristics, } = req.body;
        ...
        const colorsWithFiles = parsedColors.map((color) => {
            ...
            for (let i = 0; i < color.images.length && fileIndex < files.length; i++) {
...
const updateProduct = async (req, res) => {
    ...
    try {
        const { id } = req.params;
        const { name, brand, Category, Sub_Category, typeOfShoes, productDesc, price, availability, offer, size, feetFirstFit, footLength, technicalData, Company, gender, question, colors, characteristics, // Add characteristics to destructuring
         } = req.body;
        ...
        const colorsWithFiles = parsedColors.map((color) => {
            ...
            if (color.id) {
                const existingColor = existingProduct.colors.find((ec) => ec.id === color.id);
                if (existingColor) {
...
const queryProducts = async (req, res) => {
    try {
        const { search, sortBy = "createdAt", sortOrder = "desc", limit = "10", page = "1", question, size, } = req.query;
        ...
        const orderBy = sortQuery(sortBy, sortOrder);
        const { itemsPerPage, currentPage, skip } = paginate(limit, page);
        ...
        if (parsedSize.length > 0) {
            sizeFilteredProducts = allMatchingProducts.filter((product) => {
                ...
                try {
                    ...
                    return parsedSize.some((sz) => db_size.some((dbEl) => dbEl["size"] == sz["size"]));
                }
                catch (err) {
        ...
        if (question) {
            const question_json = JSON.parse(question);
            const inputAnswers = question_json["answers"];
            filteredProducts = sizeFilteredProducts.filter((product) => {
                try {
                    const productQuestion = JSON.parse(product.question);
                    const productAnswers = productQuestion["answers"];
                    return inputAnswers.every((inputAnswer) => productAnswers.some((productAnswer) => productAnswer["question"] === inputAnswer["question"] &&
...
const deleteProduct = async (req, res) => {
    ...
    try {
        ...
        for (const color of product.colors) {
            for (const image of color.images) {
                const filePath = path_1.default.join(__dirname, "../../uploads", image.url);
                if (fs_1.default.existsSync(filePath)) {
                    fs_1.default.unlinkSync(filePath);
                }
            }
        ...
        const productWithImageUrls = {
            ...
            characteristics: product.characteristics
                ? product.characteristics
                    .map((id) => {
                    const item = data_1.data.find((item) => item.id === id);
                    return item
                        ? {
                            ...item,
                            image: (0, base_utl_1.getImageUrl)(`/assets/KeinTitel/${item.image}`),
...
const characteristicsIcons = async (req, res) => {
    try {
        const iconsWithUrls = data_1.data.map((icon) => ({
            ...icon,
            image: (0, base_utl_1.getImageUrl)(`/assets/KeinTitel/${icon.image}`),
        }));
        res.status(200).json({
            success: true,
            data: iconsWithUrls,
...
exports.characteristicsIcons = characteristicsIcons;
const getCategorizedProducts = async (req, res) => {
    try {
        ...
        const categoryGroups = await Promise.all(categoryCounts.map(async ({ Category }) => {
            const products = await prisma.product.findMany({
                where: {
                    Category,
                    OR: [
        ...
        const subcategoryGroups = await Promise.all(subcategoryCounts
...
exports.getCategorizedProducts = getCategorizedProducts;
