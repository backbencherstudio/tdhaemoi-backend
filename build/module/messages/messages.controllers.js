...
exports.deleteSingleMessage = exports.permanentDeleteMessages = exports.getMessageById = exports.getFavoriteMessages = exports.setToFavorite = exports.getReceivedMessages = exports.getSentMessages = exports.createMessage = void 0;
...
const createMessage = async (req, res) => {
    try {
        ...
        const missingField = ["email", "subject", "content"].find((field) => !req.body[field]);
        if (missingField) {
            res.status(400).json({
                success: false,
        ...
        if (!validator_1.default.isEmail(recipientEmail)) {
            res.status(400).json({
                success: false,
        ...
        (0, emailService_utils_1.sendEmail)(recipientEmail, subject, content);
        const responseData = {
            id: newMessage.id,
            ...
            recipient: newMessage.recipient
                ? {
                    ...
                    image: newMessage.recipient.image
                        ? (0, base_utl_1.getImageUrl)(`/uploads/${newMessage.recipient.image}`)
                        : null,
                }
                : null,
...
const getSentMessages = async (req, res) => {
    try {
        ...
        const formattedMessages = sentMessages.map((message) => ({
            ...
            recipient: message.recipient
                ? {
                    ...
                    email: message.recipient.email,
                    image: message.recipient.image
                        ? (0, base_utl_1.getImageUrl)(`/uploads/${message.recipient.image}`)
                        : null,
                    role: message.recipient.role,
                }
                : null,
        ...
        const result = (0, pagination_1.getPaginationResult)(formattedMessages, total, { page, limit });
        res.status(200).json({
            success: true,
            message: "Sent messages found successfully",
            data: result.data,
        ...
        const formattedMessages = receivedMessages.map((message) => ({
            ...
            sender: message.sender
                ? {
                    ...
                    email: message.sender.email,
                    image: message.sender.image
                        ? (0, base_utl_1.getImageUrl)(`/uploads/${message.sender.image}`)
                        : null,
                    role: message.sender.role,
                }
                : null,
...
const setToFavorite = async (req, res) => {
    try {
        const { id: userId } = req.user;
        ...
        const responseData = {
            ...
            recipient: message.recipient ? {
                ...
                name: message.recipient.name,
                email: message.recipient.email,
                image: message.recipient.image ? (0, base_utl_1.getImageUrl)(`/uploads/${message.recipient.image}`) : null,
                role: message.recipient.role,
            } : null,
            recipientEmail: message.recipientEmail,
        };
...
exports.setToFavorite = setToFavorite;
const getFavoriteMessages = async (req, res) => {
    try {
        const { id: userId } = req.user;
        const { page, limit } = (0, pagination_1.getPaginationOptions)(req);
        const search = typeof req.query.search === 'string' ? req.query.search : undefined;
                ...
                sender: visibility.message.sender
                    ? {
                        ...
                        image: visibility.message.sender.image
                            ? (0, base_utl_1.getImageUrl)(`/uploads/${visibility.message.sender.image}`)
                            : null,
                ...
                recipient: visibility.message.recipient
                    ? {
                        ...
                        image: visibility.message.recipient.image
                            ? (0, base_utl_1.getImageUrl)(`/uploads/${visibility.message.recipient.image}`)
                            : null,
...
exports.getFavoriteMessages = getFavoriteMessages;
...
const permanentDeleteMessages = async (req, res) => {
    try {
        const { id: userId } = req.user;
...
exports.permanentDeleteMessages = permanentDeleteMessages;
...
//# sourceMappingURL=messages.controllers.js.map